...
export C_PATH=/usr/local/etc/scanoss/components
export LOG_DIR=/var/log/scanoss
export L_PATH="${LOG_DIR}/components"
...
if ! cp scanoss-components-api.sh /usr/local/bin ; then
  echo "components startup script copy failed"
  exit 1
fi
