...
# Config goes into: /usr/local/etc/scanoss/components
# Logs go into: /var/log/scanoss/components
...
CONF_DIR=/usr/local/etc/scanoss/components
LOGS_DIR=/var/log/scanoss/components
CONF_DOWNLOAD=https://raw.githubusercontent.com/scanoss/components/main/config/app-config-prod.json
...
if [ "$FORCE_INSTALL" -eq 1 ]; then
  ...
  read -p "Install Components API $ENVIRONMENT (y/n) [n]? " -n 1 -r
  echo
  if [[ $REPLY =~ ^[Yy]$ ]]; then
...
if [ -n "$ENVIRONMENT" ] ; then
  SC_SERVICE_FILE="scanoss-components-api-${ENVIRONMENT}.service"
  SC_SERVICE_NAME="scanoss-components-api-${ENVIRONMENT}"
fi
...
if ! cp $SCRIPT_DIR/scanoss-components-api.sh /usr/local/bin ; then
  echo "Components api startup script copy failed"
  exit 1
fi
if ! chmod +x /usr/local/bin/scanoss-components-api.sh ; then
  echo "Components api startup script permissions failed"
  exit 1
fi
...
BINARY=scanoss-components-api
if [ -f "$SCRIPT_DIR/$BINARY" ] ; then
  echo "Copying app binary to /usr/local/bin ..."
  if ! cp $SCRIPT_DIR/$BINARY /usr/local/bin ; then
    echo "copy $BINARY failed"
    echo "Please make sure the service is stopped: systemctl stop scanoss-components-api"
    exit 1
  fi
  if ! chmod +x /usr/local/bin/$BINARY ; then
    echo "execution permission on $BINARY failed"
    echo "Please make sure the use can set the binary as executable: chmod +x /usr/local/bin/scanoss-components-api"
  fi
else
  echo "Please copy the Components API binary file into: /usr/local/bin/$BINARY"
fi
...
echo "Count the number of running scans using: pgrep -P \$(pgrep -d, scanoss-components-api) | wc -l"
echo